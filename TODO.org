...
pages.
** DONE A navigational sidebar (and also maybe a pretty heading banner?)
   CLOSED: [2010-10-28 Thu 19:04]
would be nice.
...
* DONE Make "dialogical logic" appear somewhere in the page
  CLOSED: [2010-10-28 Thu 15:15]
  For search enging harvesting.
* DONE Flesh out the "about this site" page
  CLOSED: [2010-10-28 Thu 18:41]
  a link to my email, maybe the github stuff, and some details about how the games were coded.
* TODO Display the initial state of the game in the cutoff and hard negative search pages
  Right now, it just says "cutoff!" and "ouch!".  It would be useful
  if the game from which the failed search began were displayed.
* TODO Bug in search for winning strategy
  Example discovered by Sara:
  - ruleset: Peirce with E+modified D10 rules.
  - do Opponent's only move, then Proponent's.
  - then, have O defend against that move
  - then change your mind, hit backspace to go back a page in your browser
  - go down to search for a win,
  - search for a winning strat up to 15.

  The result should be that the search is cut off, but instead we get
  that the formula is provably invalid.
  
* TODO Implement Fermüller-style Ei-dialogues
  These are rather different from Felscher-style dialogues, which is
  what we've been using all along.  In trying to make sense of
  Fermüller's "Parallel dialogue games and hypersequents for
  intermediate logics", I'm having some trouble understanding
  precisely what kinds of moves are permitted, especially for
  Proponent (for Opponent I have better intuitions about what moves
  are permitted because the E rule is in force).

  How to present these?  We may need to offer two buttons: given an
  initial formula, a translation, and a ruleset, choose to play either
  a Felscher-style game or a Fermüller-style game.

  Fermüller games have the further property of permitting formulas to
  be initially granted before the dialogue begins.  We'd need some way
  of dealing with these.
* DONE Ensure that there's a "Contact the site maintainer" footer at the bottom of every page.
  CLOSED: [2010-10-28 Thu 19:14]
