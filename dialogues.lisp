...
(defun start-dialogue (initial-statement)
  ...
  (let ((response nil)
	...
	(setq response (read t nil nil)))
      (if (eq response 'a)
	  (format t "Attack which statement? (Your response should be a number between 0 and ~A.) " turn-number)
	  (format t "Defend against which attack? (Your response should be a natural number between 0 and ~A. " turn-number))
